  ...
  try {
    ...
    const stagesRes = await pool.query(
      `SELECT id, stage_order AS "order", name, description, department, assigned_role AS "assignedRole",
              default_sla_days AS "slaDays", required_documents AS "requiredDocuments"
  ...
  try {
    ...
    for (const stage of stagesRes.rows) {
      await pool.query(
        ...
        [wfId, stage.stage_order, stage.name, stage.description, stage.department, stage.assigned_role, stage.default_sla_days, stage.is_mandatory, stage.required_documents]
      );
    }
    ...
    res.json({ success: true, workflowId: wfId });
  } catch (error) {
    next(error);
  }
};
...
export const addStage = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { projectId } = req.params;
    const { name, description, department, assignedRole, assignedOfficerId, slaDays, requiredDocuments } = req.body;
  ...
  try {
    ...
    if (firstStage.rows.length > 0) {
      ...
      await pool.query(
        ...
        [projectId, wfId, fs.id, fs.stage_order, fs.name, fs.assigned_officer_id, fs.department, fs.sla_days, dueDate.toISOString(), fs.required_documents]
      );
    }
